 ...
 * at — see `resendConfirmationEmail`'s doc comment.
 */
...
/**
 * Send the sign-up confirmation email via `supabase.auth.resend()` right before the wizard
 * moves on to step 4 ("Check your inbox").
 *
 * This is now the ONE AND ONLY confirmation-email send in the wizard: step 1
...
export const saveBuildProfile = createAction(buildProfileSchema, async (input) => {
  ...
  await resendConfirmationEmail(supabase, user.email);
