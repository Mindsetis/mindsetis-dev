...
/**
 * Extended Mindsetter onboarding — step 5/5 "Make your profile shine."
 * (`docs/mindsetter-extended-onboarding.md` section 6). The last CORE step — picks which of the
 * 7 optional blocks (section 6/7) the caller wants to fill in now; the picked ones are handed to
 * the first block screen via `lib/mindsetter-onboarding/blocks.ts`'s query-param handoff rather
 * than persisted here (those block screens themselves are the next slice, not built yet).
 *
...
export default async function MindsetterOnboardingShinePage({ params }: ShinePageProps) {
  ...
  const [{ data: mindsetterProfile }, { data: sessionSettings }] = await Promise.all([
    supabase
      ...
      .select(
        'roles, superpowers, help_with, promo_video, numbers, reel_life, wins, my_way, fckups, philosophy',
      )
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] px-4 pt-4 pb-20 sm:px-6 md:pt-6 md:pb-[150px] lg:px-[70px]">
      <RegistrationStepHeader
        backHref="/mindsetter-onboarding/session"
        step={5}
        total={TOTAL_STEPS}
        label={t('common.stepLabel', { step: 5, total: TOTAL_STEPS })}
      />
