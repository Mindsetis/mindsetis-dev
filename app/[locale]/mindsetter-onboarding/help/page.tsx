 ...
 * plain flat array so that step can read titles straight off it.
 *
 * `HelpForm`'s submit navigates to `/mindsetter-onboarding/session` (step 4/5, `../session/page.tsx`).
 *
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] px-4 pt-4 pb-20 sm:px-6 md:pt-6 md:pb-[150px] lg:px-[70px]">
      <RegistrationStepHeader
        backHref="/mindsetter-onboarding/superpowers"
        step={3}
        total={TOTAL_STEPS}
        label={t('common.stepLabel', { step: 3, total: TOTAL_STEPS })}
      />

      <div className="mx-auto flex w-full max-w-[640px] flex-col gap-6">
        <div className="flex items-center gap-2">
          <Briefcase className="size-6 text-primary" aria-hidden="true" />
          <h1 className="font-display text-h1 text-foreground md:text-h3">{t('help.title')}</h1>
        </div>
